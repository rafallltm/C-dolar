...
float getExchangeRate(const char *api_key) {
    ...
    if (start != NULL) {
        ...
        fprintf(stderr, "Erro: Não foi possível encontrar a taxa de câmbio BRL na resposta da API.\n");
    }
...
int main() {
    ...
    if (scanf("%f", &reais) != 1 || reais < 0) {
        printf("Erro: Valor inválido. Por favor, insira um número positivo.\n");
        return 1;
    }
